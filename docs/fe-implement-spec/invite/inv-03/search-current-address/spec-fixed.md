...
| `maximumAge`                     | 180,000ms     | 3분. 브라우저가 방금 잡아둔 좌표를 재사용한다 (2026-08-12 변경, 아래 주석 참고).                                                |
...
> **`maximumAge` 를 0에서 3분으로 바꿨다 (2026-08-12).**
>
> 초안의 근거는 "'현재 위치로 찾기'는 지금을 요청하는 행동이므로 캐시를 쓰지 않는다" 였다.
> 그 논거는 버튼을 **처음** 누를 때는 맞지만, 위치 확인 화면은 닫으면 언마운트되므로
> 검색 화면을 오갈 때마다 고정밀 측정을 처음부터 다시 한다. 실내에서는 매번 수 초가 걸리고,
> 사용자에게는 "열 때마다 로딩"으로 보인다.
>
> 3분 이내의 최근 좌표라도 이동 속도와 측정 환경에 따라 실제 위치와 차이가 날 수 있다.
> 이 화면은 애초에 **지도를 끌어 교정하는** UI이고, `accuracy` 가 100m를 넘으면 §7의 정확도
> 안내도 붙으므로 빠른 재진입을 우선하고 사용자가 최종 위치를 확인하게 한다.
>
> 별도 좌표 캐시를 만들지 않고 브라우저에 위임한다. §4-5의 "좌표를 세션에 보존하지 않는다"
> 는 애플리케이션 상태에 좌표를 별도로 저장하지 않는다는 뜻이며, 브라우저가 Geolocation API
> 정책에 따라 최근 좌표를 재사용하는 것과는 별개다.
...
> **도로명이 비는 빈도가 생각보다 높다 (후속 과제, 2026-08-11 관측).**
>
> `coord2Address` 의 `road_address` 는 좌표가 **도로명이 부여된 필지에 걸릴 때만** 온다.
> 몇 미터만 움직여도 켜졌다 꺼졌다 하고, 지번은 매번 온다. 같은 지점에서 배달 앱들은
> 도로명·지번을 함께 보여주므로 보정 단계가 따로 있는 것으로 보인다.
>
> 보정안: `road_address` 가 `null` 이면 지번으로 `addressSearch` 를 한 번 더 호출한다.
> `addressSearch` 는 **필지 기준**이라 좌표 기준인 `coord2Address` 가 놓치는 도로명을 메운다.
> 대신 호출량이 늘어 R7(쿼터)과 지연이 함께 커진다.
>
> **1차 범위에서는 하지 않는다.** 지번 폴백만으로 기능·서버 계약(`DepartureRequest.address`)에
> 문제가 없고, 화면도 `min-h` 로 1줄·2줄 높이를 고정해 깜빡임을 흡수한다.
>
> 서버로 넘길지 여부는 §8과 함께 판단한다 — 아래 참고.
...
> **서버로 넘길 만한가 (§6-2 도로명 보정과 함께 판단).**
>
> 넘길 이유는 있다 — 좌표→주소는 **결과가 거의 변하지 않아 서버 캐시가 잘 듣는다.** 쿼터(R7)를
> 한곳에서 관리할 수 있고, 도로명 보정 같은 정규화 규칙도 한곳에 모인다. 클라이언트가 두 번
> 호출하는 대신 서버가 한 번 답하면 왕복도 줄어든다.
>
> 그래도 **1차에서는 요청하지 않는다.** 지도 SDK를 어차피 클라이언트가 띄우고 있어 추가 의존이
> 없고, 우리 서버를 한 번 더 거치면 `idle` 마다 왕복이 늘어난다. 무엇보다 이 결정은 이미 확정된
> 계약이라 도로명 보정 하나로 뒤집을 무게가 아니다.
>
> **요청하게 된다면 "도로명 보정"이 아니라 "좌표 → 정규화된 주소 + 캐시" 엔드포인트 하나로**
> 통째로 넘기는 것이 맞는 경계다. 쿼터가 실제로 문제가 된 뒤에 판단한다.
