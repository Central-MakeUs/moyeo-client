...
6. **당일 처리 — 현재 시각과 조정 리드타임** (→ #120)
   - 캘린더와 시간표 모두 **서버 기준 날짜만** 보고 비활성을 판정한다. 캘린더는 오늘을 항상
     선택 가능으로 두고, 시간표는 오늘 열을 통째로 열어둔다.
   - 응답할 시간이 남지 않은 후보는 조율 대상이 될 수 없으므로, 현재 시각과 모임 조정에 필요한
     리드타임까지 반영해야 한다.
   - 확정이 필요한 것: ① 리드타임 기준이 `deadlineMinutes` 연동인지 고정값인지 ② `noDeadline`일
     때의 처리 ③ 캘린더와 시간표의 정합성(오늘을 통째로 막을지, 유효 시간대가 남으면 허용할지).
   - ③이 선행 결정이다. 캘린더 단독으로 정하면 시간표와 어긋난다.

### 확정되어 §9에서 내려온 항목

| 항목               | 확정 내용         | 근거                                              |
| ------------------ | ----------------- | ------------------------------------------------- |
| 서비스 기준 시간대 | `Asia/Seoul` 고정 | `docs/features/INV-02/spec-fixed.md` §12-1        |
| 최대 후보 날짜     | 21일              | `docs/features/INV-02/spec-fixed.md` §12-2 · §5-3 |
