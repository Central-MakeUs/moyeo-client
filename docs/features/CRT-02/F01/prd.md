...
**목적**: 모임 생성자가 조율 후보 날짜 묶음을 빠르게(드래그) 지정한다. 3주 창(window) 안에서만 선택 가능.
...
- **US-4** 생성자로서, **3주를 넘게 끌면** 자동으로 3주까지만 잡히고 **이유를 안내**받고 싶다.
...
드래그·페인트 모드·연속 범위 계산·3주 clamp·초과 안내(토스트)를 어디에 얹을지 정해야 한다.
spec은 "3주 제한·비활성 규칙을 하드코딩하지 말고 **prop으로 주입**해 재사용"을 명시한다.
...
interface DraggableCalendarProps {
  ...
  maxRangeDays?: number; // 최대 스팬(칸). CRT-02 = 21. 없으면 무제한
  isDateDisabled?: (date: Date) => boolean; // 비활성 판정(오늘 이전 등)
  onRangeExceeded?: () => void; // 3주 초과 시 1회 호출(토스트 트리거)
...
**Context** 토스트 컴포넌트가 코드베이스에 아직 없다. "3주 내만 선택 가능" 안내가 필요.
**Decision** `DraggableCalendar`는 토스트를 직접 렌더하지 않고 **`onRangeExceeded` 콜백만 노출**한다. 실제 토스트 UI는 소비처(CRT-02)/공용 토스트 시스템 책임. F01 범위에선 콜백 계약 + Storybook 목(mock)으로 검증.
...
- **공용 토스트 시스템 구축** — F01은 `onRangeExceeded` 콜백까지. 실제 토스트 UI/스택 관리는 별도 과제.
- **키보드 다중선택·접근성 고급 인터랙션** — MVP는 포인터/탭 중심. (RDP 기본 포커스 이동은 유지)
- **선택 개수(count) 제한** — 제한은 스팬(창) 기준만. 개수 상한 없음.
- **연속 기간이라는 도메인 의미** — 최종 산출은 날짜 집합. "기간"으로 재해석하지 않음.

## 5. 용어 정의

[`spec-fixed.md` §2](./spec-fixed.md)의 Ubiquitous Language를 그대로 따른다:
셀 / 선택 / 페인트 / 페인트 모드(`select`·`deselect`) / 드래그 범위 / 비활성 / 최대 기간(스팬) / 후보 날짜.
