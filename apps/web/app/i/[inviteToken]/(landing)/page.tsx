...
import { fetchInvitationResult, InviteLandingPage, type InvitePageProps } from '@/_pages/invite';
...
export default async function InvitePage({ params }: InvitePageProps) {
  ...
  const result = await fetchInvitationResult(inviteToken);

  if (result.status === 'not-found') notFound();

  const invitation = result.status === 'success' ? result.invitation : null;
