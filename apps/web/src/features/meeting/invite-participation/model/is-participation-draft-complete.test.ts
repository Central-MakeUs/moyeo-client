...
describe('isParticipationDraftComplete', () => {
  ...
  it('SCHEDULE_ONLY이고 scheduleResponse가 null이면 false를 반환한다', () => {
    expect(isParticipationDraftComplete(EMPTY, 'SCHEDULE_ONLY')).toBe(false);
  });
  ...
  it('PLACE_ONLY이고 departure만 있고 transportationMode가 null이면 false를 반환한다', () => {
    expect(isParticipationDraftComplete({ ...EMPTY, departure: GANGNAM }, 'PLACE_ONLY')).toBe(
      false
    );
  });
