    ...
    <main className={styles.page}>
      <div className={styles.container}>
        ...
        <header className={styles.header}>
          ...
          <p>
            이 문서는 현재 레포지토리를 다시 세팅하거나 문제를 추적할 때 필요한 순서, 명령어, 참고
            URL, 의사결정을 한 곳에 모아둔 작업 기록입니다.
          </p>
        </header>
        ...
        <section className={styles.section} id="turborepo-next">
          ...
          <p>
            Turborepo의 Next.js framework guide를 기준으로 `apps/*`와 `packages/*` 구조를
            유지했습니다. `apps/web`은 WebView에 들어갈 Next.js 앱이고, `apps/docs`는 세팅 기록을
            남기는 Next.js 앱입니다.
          </p>
          <ul className={styles.list}>
            ...
            <li>
              `turbo.json`의 `dev` task는 `cache: false`, `persistent: true`로 dev server를
              다룹니다.
            </li>
            <li>
              `@repo/typescript-config`는 `./nextjs`와 `./nextjs.json` export를 모두 열어 앱
              tsconfig가 resolve되도록 했습니다.
            </li>
          </ul>
...
pnpm dev`}</pre>
        ...
        <section className={styles.section} id="bridge-types">
          ...
          <p>
            RN과 WebView가 주고받는 메시지를 먼저 `packages/types`에 고정했습니다. web 앱은
            `@repo/types`로 메시지 계약을 가져오고, `use-bridge.ts`에서 송신과 수신 hook을
            제공합니다.
          </p>
          <ul className={styles.list}>
            <li>`NativeToWebMessage`: native가 web에 보내는 인증, 디바이스, 앱 상태 메시지.</li>
            <li>
              `WebToNativeMessage`: web이 native에 요청하는 READY, 카메라, 햅틱, native navigation,
              permission 메시지.
            </li>
            <li>
              `apps/web`은 `@repo/types`와 `@repo/ui`를 모두 사용하므로 둘 다 dependencies에
              유지합니다.
            </li>
            <li>
              `@repo/ui`가 빠졌을 때 `@repo/ui/button` import 에러가 발생했고,
              `apps/web/package.json`에 다시 추가했습니다.
            </li>
          </ul>
        </section>
        ...
        <section className={styles.section} id="expo-sdk-54">
          ...
          <p>
            Expo 공식 `create-expo-app` 문서를 기준으로 `apps` 디렉터리 아래에서 native 앱을
            생성했습니다. 처음에는 `apps/native/.gitkeep` 때문에 대상 폴더가 이미 존재했으므로, Expo
            앱이 실제 생성되지 않고 placeholder만 남은 상태를 확인했습니다. 이후 실제 Expo 프로젝트
            파일이 생성되면서 `.gitkeep`은 제거했습니다.
          </p>
...
pnpm create expo-app native`}</pre>
          <ul className={styles.list}>
            ...
            <li>
              기본 템플릿에는 Expo Router, TypeScript, assets, AGENTS.md, CLAUDE.md가 포함됐습니다.
            </li>
            <li>
              SDK 56 대신 SDK 54를 사용한 이유는 create-expo-app 문서의 SDK 56 전환기 안내와 Expo Go
              실기기 검증 흐름 때문입니다.
            </li>
        ...
        <section className={styles.section} id="webview-install">
          ...
          <p>
            Expo SDK 54에 맞는 native module 버전을 받기 위해 직접 버전을 고르지 않고 Expo install
            명령을 사용했습니다. 설치 결과 `react-native-webview 13.15.0`이
...
pnpm expo install react-native-webview`}</pre>
          <ul className={styles.list}>
            ...
            <li>
              `pnpm-lock.yaml`에는 Expo SDK 54와 React Native 0.81.5에 맞는 resolution이
              반영됐습니다.
            </li>
            <li>
              이 단계는 나중에 native shell이 `apps/web`의 Next.js 화면을 로드하기 위한 기반입니다.
            </li>
          </ul>
        </section>
        ...
        <section className={styles.section} id="dev-servers">
          ...
          <h2>개발 서버 실행 방식</h2>
          <p>
            처음에는 `apps/native`에도 `dev` script를 추가해서 루트 `pnpm dev`가 web, docs, native를
            모두 실행하게 했습니다. 하지만 Expo는 QR, Expo Go, Metro command, device 연결이 있는
            interactive dev server라 Turborepo TUI 안에 묶는 것보다 별도 터미널에서 실행하는 편이
            안정적이었습니다.
          </p>
          <ul className={styles.list}>
            ...
            <li>
              `apps/native/package.json`에서는 `dev` script를 제거하고 `start: expo start`만
              유지합니다.
            </li>
          </ul>
...
pnpm dev:native`}</pre>
        ...
        <section className={styles.section} id="webview-preview">
          ...
          <p>
            Expo starter tab 화면을 `react-native-webview` 화면으로 교체해서 개발 중인 `apps/web`
            Next.js 화면이 실제 native WebView 안에서 뜨는지 확인했습니다.
          </p>
          <ul className={styles.list}>
            <li>
              `apps/native/app/(tabs)/index.tsx`는 `WebView`로 로컬 Next dev server URL을
              로드합니다.
            </li>
            <li>
              `apps/web/app/page.tsx`는 모여 WebView 확인 화면으로 바꾸고 bridge 테스트 버튼을
              제공합니다.
            </li>
            <li>
              WebView 내부에서는 `window.ReactNativeWebView.postMessage`로 native에 메시지를
              보냅니다.
            </li>
            <li>
              현재 local URL은 개발 PC의 LAN IP를 사용하는 확인용 값입니다. 추후 환경별 config로
              분리해야 합니다.
            </li>
          </ul>
        </section>
        ...
        <section className={styles.section} id="troubleshooting">
          ...
          <div className={styles.notes}>
            <article>
              ...
              <p>
                `@repo/typescript-config/nextjs.json` 경로가 package exports에 없어서 앱 tsconfig가
                공유 설정을 못 읽었고, JSX compiler option이 없다는 경고가 발생했습니다.
                `./nextjs.json` export를 추가해 해결했습니다.
              </p>
            </article>
            <article>
              <h3>Expo dependency validation 에러</h3>
              <p>
                `Body is unusable: Body has already been read`는 Metro 코드 문제가 아니라 Expo CLI가
                SDK별 native module version 목록을 네트워크로 조회하는 validation 단계에서
                발생했습니다. 임시 우회는 `EXPO_NO_DEPENDENCY_VALIDATION=1`입니다.
              </p>
            </article>
            <article>
              <h3>잘못된 위치에서 npx expo start 실행</h3>
              <p>
                루트에서 `npx expo start --clear`를 실행하면 루트를 Expo 프로젝트로 착각해서 루트
                `tsconfig.json`을 자동 생성할 수 있습니다. native 앱은 반드시 `apps/native`에서
                실행하거나 `pnpm --filter native start --clear`를 사용합니다.
              </p>
            </article>
            <article>
              <h3>Node 버전</h3>
              <p>
                현재 Node 24 계열에서는 Expo CLI 54의 fetch/undici 경로와 맞물려 validation 문제가
                반복될 수 있습니다. 반복되면 Node 22 LTS 사용을 우선 검토합니다.
              </p>
            </article>
          </div>
        </section>
